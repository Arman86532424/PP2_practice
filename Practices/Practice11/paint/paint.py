...
def draw_menu():
    ...
    start_x = WIDTH - 180   # shift left so everything fits nicely
    y = 20                  # ALWAYS inside gray bar (0–70)

    for i, c in enumerate(colors):
        x = start_x + i * 30
        rect = pygame.Rect(x, y, 25, 25)
        pygame.draw.rect(screen, c, rect)
        pygame.draw.rect(screen, "black", rect, 2)  # border for visibility
...
while True:
    ...
    if drawing:
        ...
        if tool == "pencil":
            tools.draw_pencil(canvas, color, last_pos, mouse, brush_size)
            last_pos = mouse
    ...
    if drawing:
        ...
        if tool == "line":
            ...
            pygame.draw.rect(screen, color, (*start_pos, mouse[0]-start_pos[0], mouse[1]-start_pos[1]), 2)
